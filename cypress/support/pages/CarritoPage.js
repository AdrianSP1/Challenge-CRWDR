export class CarritoPage {
    constructor() {
        ...
        this.carritoIcon = '[data-test="shopping-cart-link"]';
    ...
    removerProductoDelCarrito() {   
        cy.get(this.producto1).click();
        cy.get(this.carritoIcon).click();
        cy.get(this.itemCart).should('have.length', 1);
        cy.get(this.remover).click();
        cy.get(this.carritoItems).should('have.length', '1');/// este paso no funciona correctamente se espera que el carrito quede vacio , para que funcione correctamente debe ser 0 es parte del desafio de hacer fallar un test case
    }
}